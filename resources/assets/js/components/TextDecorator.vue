            ...
            output() {
                ...
                if (this.shouldAddMentions && this.group) {
                    this.members.forEach(member => {
                        if(member.user){
                            output = output.replace(`@${member.user.name}`, `<a href="/groups/${this.group.slug}/profiles/${member.id}" class="badge badge-pill badge-info" target="_blank">${member.user.name}</a>`);
                        }
                    })
                }
